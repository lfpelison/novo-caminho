...
path = 'pycon-2016-tutorial/data/noticias.csv'
news = pd.read_csv(path, delimiter=' ', quotechar='|', names=['index', 'message', 'risk', 'category'])
dataset = news.message
print("%d documents" % len(dataset['message']))
print("%d categories" % len(dataset['category']))
print()

labels = dataset.category.get_feature_names
...
if opts.minibatch:
    km = MiniBatchKMeans(n_clusters=4, init='k-means++', n_init=1,
                         init_size=100000, batch_size=1000, verbose=opts.verbose)
else:
